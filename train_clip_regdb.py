...
from data.dataloader import  Unlabeld_RegDBData
...
def main_worker(args):
    ...
    unlabel_dataset = Unlabeld_RegDBData(data_dir=data_path, trial=args.trial, transform=transform_test, rgb_cluster=False,
                                        ir_cluster=False)
    n_color_class = len(np.unique(unlabel_dataset.train_color_label))
    if -1 in np.unique(unlabel_dataset.train_color_label):
        n_color_class = n_color_class - 1
    n_thermal_class = len(np.unique(unlabel_dataset.train_thermal_label))
    if -1 in np.unique(unlabel_dataset.train_thermal_label):
        n_thermal_class = n_thermal_class - 1
    ...
    # checkpoint = torch.load('/home/cz/projects/checkpoint_mm/clippro/regdb/clip_pro_add_regdb_stage1.pth')
...
if __name__ == '__main__':
    ...
    args = yaml.load(open('config/config_regdb.yaml'), Loader=yaml.FullLoader)
